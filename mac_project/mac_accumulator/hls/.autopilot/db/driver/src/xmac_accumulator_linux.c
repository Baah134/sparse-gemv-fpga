...
typedef struct {
    ...
    XMac_accumulator_uio_map maps[ MAX_UIO_MAPS ];
} XMac_accumulator_uio_info;

/***************** Variable Definitions **************************************/
static XMac_accumulator_uio_info uio_info;
...
static int uio_info_read_name(XMac_accumulator_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XMac_accumulator_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XMac_accumulator_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XMac_accumulator_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XMac_accumulator_Initialize(XMac_accumulator *InstancePtr, const char* InstanceName) {
	XMac_accumulator_uio_info *InfoPtr = &uio_info;
