...
typedef struct {
    ...
    XDense_gemv_uio_map maps[ MAX_UIO_MAPS ];
} XDense_gemv_uio_info;

/***************** Variable Definitions **************************************/
static XDense_gemv_uio_info uio_info;
...
static int uio_info_read_name(XDense_gemv_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XDense_gemv_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XDense_gemv_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XDense_gemv_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XDense_gemv_Initialize(XDense_gemv *InstancePtr, const char* InstanceName) {
	XDense_gemv_uio_info *InfoPtr = &uio_info;
